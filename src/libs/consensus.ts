...
export function prevoteParticipation(raw: unknown): string {
  if (typeof raw !== 'string') return 'Non disponible';
  const match = /^BA\{\d+:[x_]+\}\s+(\d+)\/(\d+)\s+=\s+[\d.]+$/.exec(raw);
  if (!match) return 'Non disponible';
  const voted = BigInt(match[1]), total = BigInt(match[2]);
  if (total === 0n || voted > total) return 'Non disponible';
...
export function consensusStep(value: string): string {
  const names: Record<string, string> = { '1': 'NewHeight — attente du prochain bloc', '2': 'NewRound', '3': 'Propose', '4': 'Prevote', '5': 'PrevoteWait', '6': 'Precommit', '7': 'PrecommitWait', '8': 'Commit' };
  return names[value] ? `${value} · ${names[value]}` : 'Non disponible';
}
