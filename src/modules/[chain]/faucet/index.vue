...
import XitcoinFaucet from './XitcoinFaucet.vue';
...
const externalFaucet = computed(() => Boolean(chainStore.current?.faucet?.endpoint));
...
onMounted(() => {
  if (chainStore.current?.faucet && !externalFaucet.value) {
    balance();
  }
});
</script>
<template>
  <XitcoinFaucet v-if="externalFaucet" />
  <div v-else>
