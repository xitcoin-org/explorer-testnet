...
import { consensusPubkeyToHexAddress, publicValidatorMoniker, valconsToBase64 } from '@/libs';
...
const validatorSet = computed(() => {
  if (chainStore.isConsumerChain) {
    return consumerValidators.value.map((v) => {
      ...
      return {
        moniker: publicValidatorMoniker(moniker || v.moniker, chainStore.chainName),
        base64: v.base64,
      };
    });
  }
  return stakingStore.validators.map((v) => {
    ...
    return {
      moniker: publicValidatorMoniker(v.description.moniker, chainStore.chainName),
