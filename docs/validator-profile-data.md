...
Profiles distinguish unavailable data from a real zero and from fields that do not apply to the validator status. Metadata comes only from the validator description; unsafe websites and malformed email addresses remain plain text. Missing fields display `Unavailable`.
